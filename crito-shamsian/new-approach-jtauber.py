...
for row in table:
    ...
    if key == "231":
        greek_sentences["231"] = "σὺ δὲ οὔτε Λακεδαίμονα προῃροῦ οὔτε Κρήτην, ἃς δὴ ἑκάστοτε φῂς εὐνομεῖσθαι, οὔτε ἄλλην οὐδεμίαν τῶν Ἑλληνίδων πόλεων οὐδὲ τῶν βαρβαρικῶν, ἀλλὰ ἐλάττω ἐξ αὐτῆς ἀπεδήμησας ἢ οἱ χωλοί τε καὶ τυφλοὶ καὶ οἱ ἄλλοι ἀνάπηροι·"
    else:
...
for row in treebank:
    ...
    persian_translation = row["Primary translation"].strip()
    ...
    if sentence_id not in sentence_show:
        ...
        s_split = [token.strip("،؟.:«»![]؛") for token in re.split(r"[\u0020]", persian_sentences[sentence_id])]
