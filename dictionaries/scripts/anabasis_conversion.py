...
def get_entries(root):
    ...
    for i, entry in enumerate(root.xpath("ns:text//ns:div", namespaces=nsmap)):
        ...
        # conditionals below are to make sure keys are unique,
        # by catching duplicates and making them key_1, ..., key_n.

        if (
            i > 0
            and root.xpath("ns:text//ns:div", namespaces=nsmap)[i - 1].attrib["subtype"]
            == "entry"
        ):
            prev_head = (
                to_string(
                    root.xpath("ns:text//ns:div", namespaces=nsmap)[i - 1].xpath(
                        "ns:head", namespaces=nsmap
                    )[0]
                )
                .split(",")[0]
                .strip()
        ...
        if i == len(root.xpath("ns:text//ns:div", namespaces=nsmap)) - 1:
            next_head = None
        elif (
            root.xpath("ns:text//ns:div", namespaces=nsmap)[i + 1].attrib["subtype"]
            == "entry"
        ):
            next_head = (
                to_string(
        ...
        if prev_head and generate_key(prev_head) == head:
            _, key_increment = prev_head.split("_")
            entry_key = f"{entry_key}_{key_increment + 1}"
        elif next_head and generate_key(next_head) == head:
            entry_key = f"{entry_key}_0"
