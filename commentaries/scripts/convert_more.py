...
from convert_ot import get_glossae


AUTHOR_ID = "viaf2603144"

AUTHOR_DIR = pathlib.Path(__file__).parents[3] / "canonical_pdlrefwk" / "data" / AUTHOR_ID
    ...
    if path.is_dir() and path.name[-1].isdigit():
        for subpath in sorted(path.iterdir()):
            if subpath.name == "__cts__.xml":
                ...
                LABEL = " ".join([
                    root.xpath("//tei:titleStmt/tei:title", namespaces={"tei": "http://www.tei-c.org/ns/1.0"})[0].text,
                    "by",
                    root.xpath("//tei:titleStmt/tei:author", namespaces={"tei": "http://www.tei-c.org/ns/1.0"})[0].text,
                ])
                corresp = root.xpath("//*[@corresp]", namespaces={"tei": "http://www.tei-c.org/ns/1.0"})
                if corresp:
                    TEXTURN = ":".join(corresp[0].attrib["corresp"].split(":")[:-1]) + ":"
